...
public class lvlH15config : MonoBehaviour
{
    void Start()
    {
        ...
        info.stat_balls = new int[] {4,13,22,31,36,37,38,39,40,41,42,43,44,49,58,67,76}; //Статичные шарики [0..spawn.field_size*spawn.field_size]

        colorBall.timeForExplosion = 25; //Время между взрывами ColorBall'а  [9...]
        ...
        GetComponent<trickHelp>().quantity = Random.Range(3,5);//4; //Кол-во облаков (Мб сделать случайным?)
        ...
        GetComponent<info>().timerSlowdown = 0.8f; //Ускорить/замедлить таймер. 0.5 = ускорить в 2 раза, 2 = замелить в 2 раза
