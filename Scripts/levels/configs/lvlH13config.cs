...
public class lvlH13config : MonoBehaviour
{
    void Start()
    {
        spawn.field_size = 9; //Размер поля
        info.steps = 40; //Кол-во ходов, 0 - показывает кол-во ходов, >0 - убывающее кол-во ходов
        info.timersecond = 300; //Кол-во секунд, 0 - показывает время игры, >0 - убывающий таймер
        info.winBall = 2; //Победный цвет: 0-blue, 1- yellow, 2- green, 3-red;
        
        info.stat_balls = new int[] {4,12,22,32,48,58,68,76}; //Статичные шарики [0..spawn.field_size*spawn.field_size]
        ...
        GetComponent<trickHelp>().quantity = Random.Range(10,15);//4; //Кол-во облаков (Мб сделать случайным?)

        GetComponent<Bot>().enabled = true; //Вкл бота (EVA)
        info.botColor = 1; //Цвет победы бота: 0-blue, 1- yellow, 2- green, 3-red;

        SecretMech.hardLvlTrickHelpCount = 0; //Кол-во помощи трикстера
