...
public class lvlH19config : MonoBehaviour
{
    void Start()
    {
        spawn.field_size = 10; //Размер поля
        info.steps = 70; //Кол-во ходов, 0 - показывает кол-во ходов, >0 - убывающее кол-во ходов
        ...
        info.stat_balls = new int[] {4,5,14,15,24,25,34,35,40,41,42,47,48,49,50,51,52,57,58,59,64,65,74,75,84,85,94,95}; //Статичные шарики [0..spawn.field_size*spawn.field_size]

        colorBall.timeForExplosion = 10; //Время между взрывами ColorBall'а  [9...]
        colorBall.quantity = 7; //Кол-во ColorBall'а
        ...
        tortoiseBall.quantity = 7; //Кол-во TortoiseBall'а
        GetComponent<tortoiseBall>().enabled = true; //Вкл TortoiseBall'а

        GetComponent<trickHelp>().quantity = Random.Range(10,30);//4; //Кол-во облаков (Мб сделать случайным?)

        GetComponent<Bot>().enabled = true; //Вкл бота (EVA)
        info.botColor = 3; //Цвет победы бота: 0-blue, 1- yellow, 2- green, 3-red;

        SecretMech.hardLvlTrickHelpCount = 100; //Кол-во помощи трикстера

        GetComponent<info>().timerSlowdown = 3f; //Ускорить/замедлить таймер. 0.5 = ускорить в 2 раза, 2 = замелить в 2 раза
