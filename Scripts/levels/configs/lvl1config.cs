...
{  
    void Start()
    {
        ...
        GetComponent<trickHelp>().quantity = Random.Range(3,9);//3; //Кол-во облаков (Мб сделать случайным?)
        ...
        if (info.steps > 0){ //Вкл шагометра
            ...
            Debug.Log("info.configStep = "+ info.steps);
        }else{
            info.stepGo = false;
        }
        if (info.timersecond > 0){ //Вкл таймер
            ...
            Debug.Log("info.configTime = "+ info.timersecond);
        }else{
            info.timerGo = false;
        }
