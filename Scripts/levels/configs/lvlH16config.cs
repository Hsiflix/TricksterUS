...
public class lvlH16config : MonoBehaviour
{
    void Start()
    {
        ...
        info.stat_balls = new int[] {8,9,10,11,12,15,16,17,18,19,22,23,24,25,26,29,30,31,32,33,36,37,38,39,40}; //Статичные шарики [0..spawn.field_size*spawn.field_size]
        ...
        GetComponent<trickHelp>().quantity = Random.Range(1,3);//4; //Кол-во облаков (Мб сделать случайным?)

        GetComponent<Bot>().enabled = true; //Вкл бота (EVA)
        info.botColor = 3; //Цвет победы бота: 0-blue, 1- yellow, 2- green, 3-red;

        SecretMech.hardLvlTrickHelpCount = 15; //Кол-во помощи трикстера
